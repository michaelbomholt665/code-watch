...
func sortedModuleNames(imports map[string]map[string]*ImportEdge) []string {
...
func sortedImportTargets(targets map[string]*ImportEdge) []string {
	mods := make([]string, 0, len(targets))
	for mod := range targets {
		mods = append(mods, mod)
	}
	sort.Strings(mods)
	return mods
