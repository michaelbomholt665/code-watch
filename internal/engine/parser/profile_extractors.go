...
func (e *jsProfileExtractor) extractImport(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *jsProfileExtractor) extractFunction(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *jsProfileExtractor) extractClass(ctx *ExtractionContext, node *sitter.Node) bool {
	name := strings.TrimSpace(ctx.Text(node.ChildByFieldName("name")))
	if name == "" {
		return false
	}
...
func (e *jsProfileExtractor) extractMethod(ctx *ExtractionContext, node *sitter.Node) bool {
	name := strings.TrimSpace(ctx.Text(node.ChildByFieldName("name")))
	if name == "" {
		return false
	}
...
func (e *jsProfileExtractor) extractVarDecl(ctx *ExtractionContext, node *sitter.Node) bool {
	ctx.AppendLocalIdentifiers(node)
	return false
}

func (e *jsProfileExtractor) extractParams(ctx *ExtractionContext, node *sitter.Node) bool {
	ctx.AppendLocalIdentifiers(node)
	return true
}

func (e *jsProfileExtractor) extractCall(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *tsProfileExtractor) extractInterface(ctx *ExtractionContext, node *sitter.Node) bool {
	name := strings.TrimSpace(ctx.Text(node.ChildByFieldName("name")))
	if name == "" {
		return false
	}
...
func (e *tsProfileExtractor) extractTypeAlias(ctx *ExtractionContext, node *sitter.Node) bool {
	name := strings.TrimSpace(ctx.Text(node.ChildByFieldName("name")))
	if name == "" {
		return false
	}
...
func (e *tsProfileExtractor) extractEnum(ctx *ExtractionContext, node *sitter.Node) bool {
	name := strings.TrimSpace(ctx.Text(node.ChildByFieldName("name")))
	if name == "" {
		return false
	}
...
func (e *javaProfileExtractor) extractPackage(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *javaProfileExtractor) extractImport(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *javaProfileExtractor) extractClass(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindClass)
	return false
}

func (e *javaProfileExtractor) extractInterface(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindInterface)
	return false
}

func (e *javaProfileExtractor) extractEnum(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindConstant)
	return false
}

func (e *javaProfileExtractor) extractType(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindType)
	return false
}

func (e *javaProfileExtractor) extractMethod(ctx *ExtractionContext, node *sitter.Node) bool {
	kind := KindMethod
...
func (e *javaProfileExtractor) extractParameter(ctx *ExtractionContext, node *sitter.Node) bool {
	ctx.AppendLocalIdentifiers(node)
	return true
}

func (e *javaProfileExtractor) extractLocal(ctx *ExtractionContext, node *sitter.Node) bool {
	ctx.AppendLocalIdentifiers(node)
	return true
}
...
func (e *rustProfileExtractor) extractUse(ctx *ExtractionContext, node *sitter.Node) bool {
	raw := strings.TrimSpace(ctx.Text(node))
...
func (e *rustProfileExtractor) extractFunction(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindFunction)
	return false
}

func (e *rustProfileExtractor) extractType(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindType)
	return false
}

func (e *rustProfileExtractor) extractConst(ctx *ExtractionContext, node *sitter.Node) bool {
	e.addNamedDef(ctx, node, KindConstant)
	return false
}
...
func (e *rustProfileExtractor) extractLocal(ctx *ExtractionContext, node *sitter.Node) bool {
	ctx.AppendLocalIdentifiers(node)
	return true
}

func (e *rustProfileExtractor) extractCall(ctx *ExtractionContext, node *sitter.Node) bool {
...
func (e *htmlProfileExtractor) extractTag(ctx *ExtractionContext, node *sitter.Node) bool {
	tag := ""
	for i := uint(0); i < node.ChildCount(); i++ {
		child := node.Child(i)
...
func (e *cssProfileExtractor) extractImport(ctx *ExtractionContext, node *sitter.Node) bool {
	raw := ctx.Text(node)
	module := ""
	for i := uint(0); i < node.ChildCount(); i++ {
		child := node.Child(i)
...
func (e *cssProfileExtractor) extractDefinition(ctx *ExtractionContext, node *sitter.Node) bool {
