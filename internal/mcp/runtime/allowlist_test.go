...
func TestBuildOperationAllowlist_Aliases(t *testing.T) {
	cfg := &config.Config{
		MCP: config.MCP{
			OperationAllowlist: []string{"scan_once", "secrets.scan", "secrets.list", "detect_cycles", "graph.sync_diagrams", "system.generate_config", "system.generate_script", "system.watch", "query.modules", "report.generate_markdown"},
		},
	}
	...
	if !allowlist.Allows(contracts.OperationSecretsScan) {
		t.Fatalf("expected secrets.scan allowed")
	}
	if !allowlist.Allows(contracts.OperationSecretsList) {
		t.Fatalf("expected secrets.list allowed")
	}
	...
	if !allowlist.Allows(contracts.OperationReportGenMD) {
		t.Fatalf("expected report.generate_markdown allowed")
	}
