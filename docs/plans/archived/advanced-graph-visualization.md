...
| Phase | Status | Notes |
| :--- | :--- | :--- |
| **Phase 1: Importance Scoring** | ✅ Complete | `ImportanceScore` implemented in `ModuleMetrics` & `importance.go` |
| **Phase 2: C4-style Diagrams** | ✅ Complete | `GenerateC4` added with edge aggregation and violation highlighting |
| **Phase 3: New Viz Types** | ✅ Complete | Interactive D3 treemap and Mermaid sequence diagrams implemented |

### What's Implemented
- Module metrics (Fan-In, Fan-Out, Depth) in `internal/engine/graph/metrics.go`
- Importance & Complexity scoring heuristic (`Score = FanIn*2 + FanOut*1 + Complexity*0.5 + API?10`)
- C4-style architecture diagrams with auto-clustering and aggregated edges
- Violation highlighting (Red edges) in PlantUML and Mermaid
- Interactive D3 HTML Treemap generation
- Mermaid Sequence Diagram generation from call graphs

### What's Missing
- CLI wiring for new visualization types (pending integration)
- Interactive HTML report command flag
- Sequence diagram command flag
